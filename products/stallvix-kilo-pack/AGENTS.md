...
**`kilo_local_recall` classification (Gate C CORRECT_ONCE-2, read-only):** Kilo 7.4.20 `packages/opencode/src/tool/recall.ts` registers `Tool.define("kilo_local_recall")` and asks permission `"recall"` (`patterns: ["search"]`) for search mode. Same-project transcript `read` skips `ctx.ask` unless `session.projectID` differs. `Permission.disabled` maps this tool to permission key `kilo_local_recall`, not `read`/`grep`/`codebase_search`. It searches Kilo session titles/transcripts via `RecallSearch` / `Session.messages` inside the current git worktree family — it does not `read` workspace files or `.kilo-runtime-data/**` through the filesystem tools. **NOT A CONTAINMENT GAP** for this pack's secret-path / runtime-root filesystem contract. Do not deny it merely because the name looks like search. Remaining uncertainty: same-project transcript read has no permission prompt; that is session memory, not a filesystem bypass.
